...
struct clip_ctx * clip_model_load(const char * fname, const int verbosity = 1) {
    ...
    // Without scratch buffers (removed in modern ggml) all intermediate
    // activations live in buf_compute, so size it generously: model working
    // set + the old scratch requirement + headroom for the graph node array.
    const size_t mem_req = get_mem_req_by_size(new_clip)
        + get_scr_buf_req_by_size(new_clip)
        + (size_t)32 * 1024 * 1024;
...
// FurryGram: clip_image_load_from_file removed (used stb_image). Images come
// from a QImage in the wrapper, not from disk here.
...
// FurryGram: batch preprocess simplified to single-threaded (pthread removed;
// we preprocess one image at a time from the wrapper anyway).
...
bool clip_image_batch_encode(const clip_ctx * ctx, const int n_threads, const clip_image_f32_batch * imgs, float * vec,
    ...
    for (int b = 0; b < batch_size; b++) {
        ((int32_t *)cls->data)[b] = b * num_positions;
    }
    ...
    // final visual projection -> [projection_dim, batch_size]. Output
    // normalization is done in C++ after compute (see below).
    struct ggml_tensor * output = ggml_mul_mat(ctx0, model.projection, embeddings);
...
// print
    ...
    if (normalize) {
        for (int b = 0; b < batch_size; b++) {
            float * v = vec + (size_t)b * projection_dim;
            double norm = 0.0;
            for (int i = 0; i < projection_dim; i++) {
